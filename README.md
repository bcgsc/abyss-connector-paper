...
One of the main characteristics of both Sanger and second-generation sequencing methods is the use of a paired-end tag (PET) sequencing strategy. Under the PET sequencing approach, short DNA sequences are determined at each end of a DNA fragment, and the pairing relationship of the sequences is recorded for downstream use. Although the size of the sequenced DNA fragments typically varies according to a normal distribution, the pairing between reads nonetheless provides valuable distance information that has been successfully exploited by assemblers to improve both the quality and completeness of assemblies. In particular, many assembly pipelines (**cite**) incorporate *scaffolding algorithms* that map paired end reads across contigs to determine their relative distances and ordering. Scaffolding algorithms typically operate by aligning paired end reads to genome sequences ("contigs") that have first been assembled using  OLC, a de Bruijn graph, or any other assembly method.  As such, scaffolding is a step that is distinct from the rest of the genome assembly process, and many scaffolding algorithms have been implemented independently of any particular assembly pipeline (**cite**).    

As second-generation sequencing technologies have improved, it has become possible to extend the paired end reads far enough into the DNA fragment that the reads overlap in the middle. This has presented an opportunity to align the overlapping reads and combine them into a single long sequence which has alternately been referred to as a "pseudoread", "merged read", or a "super-read". Numerous tools have been developed to merge overlapping reads (**cite**), and the evaluation of these tools has demonstrated that the use of pseudoreads in place of raw reads for assembly can significantly improve the quality of assembled genomes.  As the act of merging reads is in effect building a consensus for a particular genomic location, the construction of pseudoreads can also have the side effect of correcting sequencing errors.

In principle, it is also possible to extend or merge paired end reads that do *not* overlap; however, few such tools have been published to date. One of the key challenges involved in developing such a tool is the task of determining overlaps among reads across randomly sampled genomic locations, as is done in OLC assemblers. However, this requires aligning the reads and is known to be computationally expensive. Some read extenders, such as ELOPER (**cite**) and the MaSuRCA assembler (**cite**), have adopted a hash-table based  approach that used fixed-size overlaps similar to the de Bruijn graph assembly method.
