...
The task of reconstructing the genome sequence of an organism from the short DNA sequences ("reads") produced by DNA sequencing machines, without reference to any previously assembled genome, is known as the *de novo assembly* problem. Despite the introduction of many new sequencing technologies and assembly algorithms over the past decade, the de novo assembly of large genomes currently remains a major challenge not only in terms of the correctness and completeness of the assembled sequences, but also with respect to the computational scalability of the algorithms.
